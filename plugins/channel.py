...
async def send_movie_updates(bot, file_name, caption, file_id):
    try:
        ...
        caption_message = f"<b><blockquote>#New_File_Added</b> </blockquote>\n\n🍿 𝗧𝗶𝘁𝗹𝗲 : <code>{movie_name}</code>\n\n🔊 𝗟𝗮𝗻𝗴𝘂𝗮𝗴𝗲 : {language}\n\⚙️ 𝗤𝘂𝗮𝗹𝗶𝘁𝘆 : {quality}" 
        search_movie = movie_name.replace(" ", '-')
        movie_update_channel = await db.movies_update_channel_id()    
        btn = [[
            InlineKeyboardButton('📂 ɢᴇᴛ ғɪʟᴇ 📂', url=f'https://telegram.me/{temp.U_NAME}?start=getfile-{search_movie}')
        ],[
            InlineKeyboardButton('♻️ ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ ♻️', url=f'https://t.me/HowTOdo4/11')
        ]]
